...
class PlaybookEngine:
    def __init__(self, playbooks: List[BasePlaybook]) -> None:
        self.playbooks: List[BasePlaybook] = playbooks
    ...
    def process(self, incidents: List[Incident]) -> List[Dict[str, Any]]:
        ...
        for incident in incidents:
            for playbook in self.playbooks:
                try:
                    result: List[Dict[str, Any]] = playbook.run(incident)

                    if result:
                        actions.extend(result)

                except Exception as e:
                    print(f"[Playbook Error] {playbook.__class__.__name__}: {e}")
